...
let movimientos = 0;
//tiempo inicial y temporizador
let TiempoInicial = 60; // puedes ajustar
...
function comprobarRespuesta(opElegida) {
  movimientos++; // cada respuesta es un movimiento
...
function terminarJuego() {
  ...
  // ---- LLAMAMOS A onEndGame ----
  const user = JSON.parse(localStorage.getItem("user")); // corregido
  if (user) {
    onEndGame({
      moves: movimientos,
      score: cantidadAcertadas,
      gameType: "educativas",
      gameName: "quiz-Animales",
      seconds: TiempoInicial - timer,
      pacienteId: user.identificacion,
    });
  }
}
