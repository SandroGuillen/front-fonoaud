...
//Variable que guarda la cantidad de intentos restantes POR PALABRA
...
function cargarNuevaPalabra() {
  ...
  //Verificar si ya no hay más palabras en el array
  ...
  //setemos los intentos POR PALABRA
  intentosRestantes = 5;
  ...
  if (isLetter(event.key)) {
    ...
    if (letrasIngresadas.lastIndexOf(event.key.toUpperCase()) === -1) {
      ...
      if (acerto == true) {
        ...
        //No acerto, decremento los intentos restantes POR PALABRA
        intentosRestantes = intentosRestantes - 1;
        document.getElementById("intentos").innerHTML = intentosRestantes;

        //controlamos si ya acabo todas la oportunidades POR PALABRA
        if (intentosRestantes <= 0) {
          for (i = 0; i < arrayPalabraActual.length; i++) {
            divsPalabraActual[i].className = "letra pintarError";
          }

          // Pasar a la siguiente palabra después de un breve delay
          setTimeout(() => {
            if (arrayPalabras.length > 0) {
              cargarNuevaPalabra();
            } else {
