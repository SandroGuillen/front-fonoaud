...
async function renderRow(data) {
  table.innerHTML += `
    <tr>
        ...
        <td>${calcularEdad(data.fechaNacimiento)}</td>
        <td>${data.correo}</td>
        <td>${data.telefono}</td>
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  $("#pacientesTable").on("click", ".historial-btn", function () {
    ...
    if (paciente) {
      $("#pacienteNombreHistorial").text(
        `${paciente.nombres} ${paciente.apellidos}`
      );
      $("#pacienteInfoHistorial").html(`
                <strong>Documento:</strong> ${paciente.tipoDocumento} ${
        paciente.documento
      }<br>
                <strong>Edad:</strong> ${calcularEdad(
                  paciente.fechaNacimiento
                )} años<br>
                <strong>Acudiente:</strong> ${paciente.acudiente} (${
        paciente.parentesco
      })
            `);
  ...
  $("#exportBtn").click(function () {
    ...
    Swal.fire("Exportar", "Los datos serán exportados a Excel", "info");
  });
